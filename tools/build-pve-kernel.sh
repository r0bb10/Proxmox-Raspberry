...
zfs_head=$(git -C "$zfs" rev-parse HEAD)
(
    cd "$zfs"
    sh autogen.sh
...
dpkg-deb --root-owner-group --build "$package" "$output/linux-image-${version}_arm64.deb"
...
EOF
sha256sum "$output/linux-image-${version}_arm64.deb" > "$output/linux-image-${version}_arm64.deb.sha256"
